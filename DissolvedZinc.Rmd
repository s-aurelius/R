...
parameter <- 'Zinc - Water - Dissolved'
...
```
We will pass MFR, IND, Impee, roofs,  to our mixed model
...
```{r}
PQL <- glmmPQL(new_result_value ~ MFR+IND+elev+roofs, data=s8data, random = ~1|Location,family = gaussian(link= 'log'))
...
```{r}
PQL <- glmmPQL(new_result_value ~ MFR, data=s8data, random = ~1|Location,family = gaussian(link= 'log'))
