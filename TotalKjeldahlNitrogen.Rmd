...
parameter <- 'Total Kjeldahl Nitrogen - Water - Total'
...
```
We will pass MFR, IND, pm25, roofs,  to our mixed model
...
```{r}
PQL <- glmmPQL(new_result_value ~ MFR+IND+pm25+roofs, data=s8data, random = ~1|Location,family = gaussian(link = 'log'))
...
PQL2 <-PQL
lm.compare <- lm(log(new_result_value) ~ MFR+IND+pm25+roofs, data = s8data)
summary(PQL2)
summary(lm.compare)
...
glme.equation <- tibble(x = s8data$IND,y = exp(6.239803+0.902110*s8data$MFR+2.596632*s8data$IND +0.559281*s8data$pm25+1.671876*s8data$roofs))
lm.equation <- tibble(x = s8data$IND,y = exp(5.6819              +0.2129              *s8data$MFR +3.8683  
                                             *s8data$IND + 0.9203     * s8data$pm25 +2.3927 *s8data$roofs))
...
```{r}
#compare with a linear regression model 
glme.equation <- tibble(x = s8data$roofs,y = exp(6.239803+0.902110*s8data$MFR+2.596632*s8data$IND +0.559281*s8data$pm25+1.671876*s8data$roofs))
lm.equation <- tibble(x = s8data$roofs,y = exp(5.6819              +0.2129              *s8data$MFR +3.8683  
                                             *s8data$IND + 0.9203     * s8data$pm25 +2.3927 *s8data$roofs))
