...
parameter <- 'Copper - Water - Dissolved'
...
Pass ghm, pm25, Impee, roofs to mixed model
...
roofs, pm25, ghm insignificant and were removed

```{r}
PQL <- glmmPQL(new_result_value ~ Impee, data=s8data, random = ~1|Location,family = gaussian(link = 'log'))
...
ran without log function, pm25 was only significant factor, but p-value higher than Impee with log function
...
glme.equation <- tibble(x = s8data$Impee,y = exp(0.2548256+2.4641978*s8data$Impee))
lm.equation <- tibble(x = s8data$Impee,y = exp(0.1601              +2.1143             *s8data$Impee))
