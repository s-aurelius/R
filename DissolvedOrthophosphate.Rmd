...
parameter <- 'Ortho-phosphate - Water - Dissolved'
...
```
We will pass ghm, ndbi, elev, roofs,  to our mixed model
...
```{r}
PQL <- glmmPQL(new_result_value ~ roofs+elev+ndbi+ghm, data=s8data, random = ~1|Location,family = gaussian(link='log'))
...
```{r}
PQL <- glmmPQL(new_result_value ~ roofs+ghm, data=s8data, random = ~1|Location,family = gaussian(link='log'))
#remove elev, ndbi
